...
import {
  dsDropdownCheckboxItem,
  dsDropdownContent,
  dsDropdownItem,
  dsDropdownLabel,
  dsDropdownShortcut,
  dsDropdownSubTrigger,
...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      "flex cursor-pointer select-none items-center gap-2 rounded-sm outline-none [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
      dsDropdownSubTrigger,
      inset && "pl-8",
      className,
    )}
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn(dsDropdownLabel, inset && "pl-8", className)}
    {...props}
  />
));
...
const DropdownMenuShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
  return (
    <span className={cn(dsDropdownShortcut, className)} {...props} />
  );
};
